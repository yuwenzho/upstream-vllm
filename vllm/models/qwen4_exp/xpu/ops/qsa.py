...
"""Triton kernels for the Qwen4Exp weight-free QSA path (XPU variant)."""
...
def qsa_mqa_paged(
    ...
    _validate_mqa(q)
    if not (q.is_cuda or q.is_xpu) or not HAS_TRITON:
...
def expand_qsa_block_indices_cuda(
    ...
    if not (block_indices.is_cuda or block_indices.is_xpu) or not HAS_TRITON:
...
def qsa_store_cache_rows(
    ...
    if not (cache.is_cuda or cache.is_xpu) or not HAS_TRITON:
...
def qsa_compress_groups_with_ratio(
    ...
    if not (raw_keys.is_cuda or raw_keys.is_xpu) or not HAS_TRITON:
