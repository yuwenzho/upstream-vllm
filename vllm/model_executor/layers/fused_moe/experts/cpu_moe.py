...
"""CPU FP8 W8A16 and MXFP4 W4A16 fused MoE experts via AMX BRGEMM."""
...
class CPUExpertsMxfp4(mk.FusedMoEExpertsMonolithic):
    ...
    Uses the AMX-based fused_experts_mxfp4_cpu kernel with MXFP4 weight
    dequantization. Weights are VNNI-prepacked and scales are repacked
    during loading.

    Monolithic because routing (select_experts) and expert computation
    (fused_experts_mxfp4_cpu) are handled together, avoiding the modular
